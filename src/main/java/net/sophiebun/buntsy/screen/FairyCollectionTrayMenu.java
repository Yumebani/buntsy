...
public class FairyCollectionTrayMenu extends AbstractContainerMenu {

    private final FairyCollectionTrayBlockEntity blockEntity;
    private final Level level;

    protected FairyCollectionTrayMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public FairyCollectionTrayMenu(int pContainerId, Inventory inv, BlockEntity entity) {
        super(ModMenuTypes.FAIRY_COLLECTION_TRAY_MENU.get(), pContainerId);
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(iItemHandler -> {
            for (int i = 0; i < 15; i++){
                this.addSlot(new OutputSlot(iItemHandler, i, 44 + (18 * (i % 5)), 17 + (18 * (i / 5))));
            }
        });
    }
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, ModBlocks.FAIRY_COLLECTION_TRAY.get());
    }
