...
public class FumeSpreaderMenu extends AbstractContainerMenu {

    private final FumeSpreaderBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    protected FumeSpreaderMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(6));
    }

    public FumeSpreaderMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.FUME_SPREADER_MENU.get(), pContainerId);
        blockEntity = ((FumeSpreaderBlockEntity) entity);
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, ModBlocks.FUME_SPREADER.get());
    }
    ...
    public int getScaledProgress() {
        ...
        return maxProgress != 0 && progress != 0 ? progress * 148 / maxProgress : 0;
    }
}
