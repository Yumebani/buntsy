...
public class FairyInfusionBenchMenu extends AbstractContainerMenu {

    private final FairyInfusionBenchBlockEntity blockEntity;
    private final Level level;

    protected FairyInfusionBenchMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public FairyInfusionBenchMenu(int pContainerId, Inventory inv, BlockEntity entity) {
        super(ModMenuTypes.FAIRY_INFUSION_BENCH_MENU.get(), pContainerId);
        checkContainerSize(inv, 10);
        blockEntity = ((FairyInfusionBenchBlockEntity) entity);
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, ModBlocks.FAIRY_INFUSION_BENCH.get());
    }
