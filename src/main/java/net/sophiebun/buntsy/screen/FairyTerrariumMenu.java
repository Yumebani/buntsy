...
public class FairyTerrariumMenu extends AbstractContainerMenu {
    ...
    protected FairyTerrariumMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
    ...
    public FairyTerrariumMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(iItemHandler -> {
            this.addSlot(new FairyInBottleSlot(iItemHandler, 0, 8, 17));
            this.addSlot(new SlotItemHandler(iItemHandler, 1, 8, 35));
            this.addSlot(new OutputSlot(iItemHandler, 2, 8, 53));

            for (int i = 3; i < 6; i++){
                this.addSlot(new SlotItemHandler(iItemHandler, i, 53, 17 + (18 * (i - 3))));
            }

            for (int i = 6; i < 21; i++){
                this.addSlot(new OutputSlot(iItemHandler, i, 80 + (18 * ((i - 6) % 5)), 17 + (18 * ((i - 6) / 5))));
            }
        });
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, ModBlocks.FAIRY_TERRARIUM.get());
    }
    ...
    public boolean isFlowerValid() {
        return data.get(3) == 1;
    }

    public int getScaledFood() {

        int food = this.data.get(0);
        int maxFood = this.data.get(1);
