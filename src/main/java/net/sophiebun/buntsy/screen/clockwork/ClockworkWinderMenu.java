...
public class ClockworkWinderMenu extends AbstractContainerMenu {
    ...
    private final ClockworkWinderEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public ClockworkWinderMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(2));
    }

    public ClockworkWinderMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.CLOCKWORK_WINDER_MENU.get(), pContainerId);
        blockEntity = ((ClockworkWinderEntity) entity);
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, ModBlocks.CLOCKWORK_WINDER.get());
    }
    ...
    public float getBurnFloat() {
        if (this.data.get(1) != -1){
            return (float)this.data.get(0) / (float)this.data.get(1);
        }
        return 0;
    }
}
