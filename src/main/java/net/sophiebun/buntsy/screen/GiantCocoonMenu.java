...
public class GiantCocoonMenu extends AbstractContainerMenu {
    ...
    private final GiantCocoonBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    protected GiantCocoonMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(1));
    }

    public GiantCocoonMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.GIANT_COCOON_MENU.get(), pContainerId);
        blockEntity = ((GiantCocoonBlockEntity) entity);
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, ModBlocks.GIANT_COCOON.get());
    }
