...
public class FairyInfusionBenchRenderer implements BlockEntityRenderer<FairyInfusionBenchBlockEntity> {

    public FairyInfusionBenchRenderer(BlockEntityRendererProvider.Context context){
    }

    @Override
    public void render(FairyInfusionBenchBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {

        ItemRenderer itemRenderer = Minecraft.getInstance().getItemRenderer();
        List<ItemStack> items = pBlockEntity.getRenderInputItems();
        ...
        ItemStack output = pBlockEntity.getFirstOutputItemStack();

        if (!output.isEmpty()){
            pPoseStack.pushPose();
            pPoseStack.translate(0.5f, 0.4f, 0.5f);
            pPoseStack.scale(0.35f, 0.35f, 0.35f);
            pPoseStack.mulPose(Axis.YP.rotationDegrees(rotations.get(4)));
            pPoseStack.mulPose(Axis.XN.rotationDegrees(270));
