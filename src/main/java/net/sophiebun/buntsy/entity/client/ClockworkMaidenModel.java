...
public class ClockworkMaidenModel<T extends Entity> extends HierarchicalModel<T> {
	...
	public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		if (maiden.isUsingItem() && maiden.getUseItem().getItem() instanceof BowItem){
			animateWalk(ClockworkMaidenAnimationDefinition.walkCycleNoArms, limbSwing, limbSwingAmount, 2f, 3.5f);}
		else {
			animateWalk(ClockworkMaidenAnimationDefinition.walkCycle, limbSwing, limbSwingAmount, 2f, 3.5f);
		}

		this.animate(maiden.windUpAnimationState, ClockworkMaidenAnimationDefinition.windupSpin, ageInTicks, 1f);
		this.animate(maiden.bowStanceAnimationState, ClockworkMaidenAnimationDefinition.bowStance, ageInTicks, 2f);
		this.animate(maiden.idleAnimationState, ClockworkMaidenAnimationDefinition.idleArms, ageInTicks, 1.5f);
		this.animate(maiden.swingAnimationState, ClockworkMaidenAnimationDefinition.attack, ageInTicks, 0.5f);
	}
